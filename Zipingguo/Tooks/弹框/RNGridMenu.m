...
CGFloat const kRNGridMenuDefaultBlur = 0.1f;
...
- (instancetype)initWithView:(UIView *)gView{
    if (self = [super init]) {
        _cornerRadius = 8.f;
        ...
        _bounces = YES;
        _gView = gView;
        orignGViewFrame = _gView.bounds;
        orignHeight = orignGViewFrame.size.height;
        [_menuView addSubview:gView];
        _menuStyle = RNGridMenuStyleView;
        [self setupItemViews];
...
- (void)viewWillLayoutSubviews {
    ...
    else if (self.menuStyle == RNGridMenuStyleView){
        [self layoutGridView];
    }
...
- (void)layoutGridView{
    CGFloat headerOffset = self.headerView.bounds.size.height;
//    self.menuView.frame = [self menuFrameWithWidth:orignGViewFrame.size.width / 320.0 * ShareApp.window.bounds.size.width height:_gView.bounds.size.height center:self.menuCenter headerOffset:headerOffset];
    float x1 = orignGViewFrame.size.width / 320.0;
    float y1 = orignGViewFrame.size.height / 568.0;
    //如果不是全屏 (x1 < 1) 则保持资深比例(orignGViewFrame)
    self.menuView.frame = [self menuFrameWithWidth:ShareApp.window.bounds.size.width * x1  height: x1 < 1 ? orignGViewFrame.size.height : ShareApp.window.bounds.size.height * y1 center:self.menuCenter headerOffset:headerOffset];
    _gView.frame = self.menuView.bounds;
    if (self.riLiView) {
        self.riLiView.frame = self.menuView.bounds;
    }
    if (self.shiJianView) {
        self.shiJianView.frame = self.menuView.bounds;
    }
}
...
- (void)showInViewController:(UIViewController *)parentViewController{
    CGPoint center = parentViewController.view.center;
    ...
    self.view.frame = ShareApp.window.bounds;
...
- (void)showAnimated:(BOOL)animated {
    ...
    self.tmpView = [[UIView alloc] initWithFrame:self.parentViewController.view.bounds];
    self.tmpView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    self.tmpView.backgroundColor = RGBACOLOR(100, 100, 100, 0.1);
...
- (void)addShiJianView:(UIView *)view{
    if (!self.shiJianView) {
        self.shiJianView = view;
        shiJianHeight = view.frame.size.height;
        view.hidden = YES;
        [self.menuView addSubview:view];
    }
}

- (void)addRiLiView:(UIView *)view{
    if (!self.riLiView) {
        self.riLiView = view;
        riLiHeight = view.frame.size.height;
        view.hidden = YES;
        [self.menuView addSubview:view];
    }
}
...
- (void)showShiJianView{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        CGRect f = orignGViewFrame;
        f.size.height = shiJianHeight;
        orignGViewFrame = f;
        [self layoutGridView];
