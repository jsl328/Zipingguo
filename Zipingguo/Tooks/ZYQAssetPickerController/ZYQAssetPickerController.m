...
- (void)setupButtons:(NSInteger)count
{
    ...
    if (count != 0) {
        [btn setFrame:CGRectMake(0,0,60,18)];
        [btn setTitle:[NSString stringWithFormat:@"发布(%ld)",(long)count] forState:UIControlStateNormal];
    }else{
        [btn setFrame:CGRectMake(0,0,40,18)];
        [btn setTitle:[NSString stringWithFormat:@"发布"] forState:UIControlStateNormal];
    }
    [btn setTitleColor:RGBACOLOR(4, 175, 245, 1) forState:UIControlStateNormal];
...
//    [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"发布", nil)
...
//    self.title = [NSString stringWithFormat:format, (long)indexPaths.count];
    self.title = @"相册";
    [self setupButtons:(long)indexPaths.count];
//     self.navigationItem.rightBarButtonItem.title = [NSString stringWithFormat:@"发布(%ld)",(long)indexPaths.count];
}
