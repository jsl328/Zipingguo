    ...
    for (int i=0; i<CFArrayGetCount(records); i++) {
        ...
        if (phoneNums) {
            for (int j=0; j<CFArrayGetCount(phoneNums); j++) {
                NSString *phone = (NSString*)CFArrayGetValueAtIndex(phoneNums, j);                if ([phone isEqualToString:phoneNum]) {
...
//        dispatch_release(sema);
    ...
    for (NSDictionary*dic in self.dataArrayDic) {
        NSString* str;
       NSString *first =[dic objectForKey:@"first"];
