...
-(void)setAutoScroll:(BOOL)autoScroll{
    ...
    if (_autoScroll && _imagesGroup.count > 1) {
        [self setupTimer];
    }
}
...
- (void)layoutSubviews
{
    ...
    CGFloat y = self.mainView.sd_height - _titleLabelHeight + (_titleLabelHeight - size.height)/2;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.titleLabel.frame = CGRectMake(0, self.frame.size.height - _titleLabelHeight, self.frame.size.width - _pageControl.frame.size.width - 20, _titleLabelHeight);
    return cell;
}
