//
//  ZifenzuXuanrenViewController.m
//  Zipingguo
//
...
#import "BianjiVC.h"
@interface ZifenzuXuanrenViewController ()<UITextFieldDelegate>
{
    //原始数据源
...
@implementation ZifenzuXuanrenViewController
...
- (void)loadData{
    ...
    [ServiceShell getDeptPersonsWithID:_ID usingCallback:^(DCServiceContext *serviceContext, ResultModelOfDeptPersonsSM *deptpersonsSM) {
        for (DeptPersonsSM *personsSM in deptpersonsSM.data) {
            ...
            if (_xuanzhongArray.count != 0) {
                for (XuanzeRenyuanModel *renyuan in _xuanzhongArray) {
                    if ([renyuan.personsSM.userid isEqualToString:personsSM.userid]) {
                        model.xuanzhong = YES;
                    }
                }
            }
...
- (void)textFieldChanged{
    ...
    if ([lang isEqualToString:@"zh-Hans"]) {
        ...
        if (!position) {
            ...
            for (XuanzeRenyuanModel *model in _dataArray) {
